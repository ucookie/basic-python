...
# 这个方法我们一般很少定义，不过我们在一些开源框架中偶尔会遇到定义这个方法的类
# 实际上，这才是“真正的构造方法”，它会在对象实例化时第一个被调用，
# 然后再调用__init__，它们的区别主要如下：
# __new__的第一个参数是cls，而__init__的第一个参数是self
# __new__返回值是一个实例，而__init__没有任何返回值，只做初始化操作
# __new__由于是返回一个实例对象，所以它可以给所有实例进行统一的初始化操作
# 由于__new__优先于__init__调用，且返回一个实例，
# 所以我们可以利用这种特性，每次返回同一个实例来实现一个单例类：
